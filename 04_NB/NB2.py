# -*- coding: utf-8 -*-
"""
Created on Fri May 17 00:00:48 2019
...
def testingNB():
    ...
    dataset = data.data
    target = data.target
    dataset[:,0]=pd.cut(dataset[:,0],2,labels=[0,1])
    dataset[:,1]=pd.cut(dataset[:,1],2,labels=[0,1])
    dataset[:,2]=pd.cut(dataset[:,2],2,labels=[0,1])
    dataset[:,3]=pd.cut(dataset[:,3],2,labels=[0,1])
...
#    print (testEntry, 'classified as: ', classifyNB(testEntry, p0V, p1V, pAb))
    ...
    for j in range(10):
        ...
        prob = 0.1
        for i in range(10):
            p0V, p1V, pAb = trainNB0(x_train[tmp:np.int(sample*prob)],y_train[tmp:np.int(sample*prob)])
...
#        print(sum(pri)/len(pri))
    print('iris朴素贝叶斯，10次十折交叉验证结果:',sum(res)/len(res))
